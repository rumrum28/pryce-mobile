export const ifPGCMemberDescription = {
  member: [
    'PRYCEGAS CLUB MEMBERSHIP IS ACTIVE',
    'You can use the price protection benefit and your 24/7 free delivery service on your order. If you have any concerns, please call #98000 toll-free.',
  ],
  notMember: [
    'Be a PRYCEGAS Club Member now',
    'Register to enjoy fixed prices on selected products over a year with 24/7 Free Delivery Service!. If you have any concerns, please call #98000 toll-free.',
  ],
}

export const Products = [
  { id: 'NOIMAGE', image: ['/images/no-image.png'] },
  {
    id: 'MEDREG',
    name: 'Medical Regulator',
    description: ['Medical regulator'],
    image: require('~/assets/png/products/MEDREG.png'),
  },
  {
    id: 'MO2105C',
    name: '105lbs Refill',
    description: ['Content only'],
    image: require('~/assets/png/products/MO2105C.png'),
  },
  {
    id: 'MO220C',
    name: '20lbs Refill',
    description: ['Content only'],
    image: require('~/assets/png/products/MO220C.png'),
  },
  {
    id: 'LPG50C',
    name: '50kg Refill',
    description: ['Content only'],
    image: require('~/assets/png/products/LPG50C.png'),
  },
  {
    id: 'CYL50',
    name: '50kg Cylinder',
    description: ['Cylinder Only'],
    image: require('~/assets/png/products/CYL50.png'),
  },
  {
    id: 'LPG22C',
    name: '22kg Refill',
    description: ['Content only'],
    image: require('~/assets/png/products/LPG22C.png'),
  },
  {
    id: 'LPG22CCOD',
    name: '22kg Orange Day Cylinder With Content',
  },
  {
    id: 'CYL22',
    name: '22kg Cylinder',
    description: ['Cylinder Only'],
    image: require('~/assets/png/products/CYL22.png'),
  },
  {
    id: 'LPG11C',
    name: '11kg Refill',
    description: ['Content only'],
    image: require('~/assets/png/products/LPG11C.png'),
  },
  {
    id: 'CYL11',
    name: '11kg Cylinder',
    description: ['Cylinder Only'],
    image: require('~/assets/png/products/CYL11.png'),
  },
  {
    id: 'LPG11CCABOD',
    name: '11kg Orange Day Bundle',
  },
  {
    id: 'LPG11CCOD',
    name: '11kg Orange Day Cylinder With Content',
  },
  {
    id: 'CYL2.7',
    name: '2.7kg Cylinder',
    description: ['Cylinder Only'],
    image: require('~/assets/png/products/CYL2.7.png'),
  },
  {
    id: 'LPG2.7CCPK',
    name: '2.7kg Cylinder with Content & Power Burner',
    description: ['Cylinder', 'Content', 'Power Burner'],
    image: require('~/assets/png/products/LPG2.7CCPK.png'),
  },
  {
    id: 'LPG2.7C',
    name: '2.7kg Refill',
    description: ['Content only'],
    image: require('~/assets/png/products/LPG2.7C.png'),
  },
  {
    id: 'LPG2.7CCPKOD',
    name: '11kg Orange Day Cylinder With Content',
  },
  {
    id: 'ACCPGS',
    name: 'Prycegas stove',
    description: ['Prycegas stove'],
    image: require('~/assets/png/products/ACCPGS.png'),
  },
  {
    id: 'ACCHRCB',
    name: 'Add-ons (Hose, Regulator, Clamp)',
    description: [
      'Add-ons (Hose, Regulator, Clamp). You can only order this product if you purchase a refill or a cylinder with content.',
    ],
    image: require('~/assets/png/products/ACCHRCB.png'),
  },
  {
    id: 'PGCM',
    name: 'Prycegas Club Membership',
    description: [
      'Prycegas Club Membership',
      'Register to enjoy fixed prices on selected products over a year with 24/7 Free Delivery Service!. If you have any concerns, please call #98000 toll-free.',
    ],
    image: require('~/assets/png/products/PGCM.png'),
  },
  {
    id: 'PGCMV',
    name: 'Prycegas Club Membership',
    description: [
      'Prycegas Club Membership',
      'Register to enjoy fixed prices on selected products over a year with 24/7 Free Delivery Service!. If you have any concerns, please call #98000 toll-free.',
    ],
  },
  {
    id: 'MO2TFT',
    name: '20lbs Cylinder with Content',
    description: ['Cylinder with Content'],
    image: require('~/assets/png/products/MO2TFT.png'),
  },
  {
    id: 'MO2TST',
    name: '105lbs Cylinder',
    description: ['Cylinder with Content'],
    image: require('~/assets/png/products/MO2TST.png'),
  },
  {
    id: 'ACCPKB',
    name: 'Prycegas Burner',
    description: ['Burner only'],
    image: require('~/assets/png/products/ACCPKB.png'),
  },
]
