...
import { LoginResponse, ProfileResponseSingle } from '~/types/apiresults'

interface PryceStoreState {
  users: any
  pryceSettings: any
  setPryceSettings: (type: string, value: string) => Promise<void>
  pryceLogin: (data: LoginResponse) => Promise<void>
  testGetAllFromPryceTable: () => void
  testGetAllFromUsersTable: () => void
  testGetAllFromPryceSettingsTable: () => void
}

const transaction = async (data: any, t: any) => {
  const db = await SQLite.openDatabaseAsync('PryceDB')
